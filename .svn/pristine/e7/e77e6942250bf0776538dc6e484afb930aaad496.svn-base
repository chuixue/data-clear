...
def temp1():
    ...
    con2 = MongoClient('192.168.3.45', 27017)
    con3 = MongoClient('192.168.3.119', 27017)
    con4 = MongoClient('101.204.243.241', 27017)
    db1 = con1['middle']
    db2 = con2['constructionDB']
    db3 = con3['jianzhu3']
    db4 = con4['jianzhu3']
#    db3=db4
    ...
    write = db4.personNew 
    lskey = P.dbKeys(db1.personNew, ['id', 'certificate'])
    lsUpdate = []
    for item in db1.personNew.find({}, lskey):
        flag = 0
        for ln in item['certificate']:            
            if ln['professional']=="给排水": flag = 1
        if flag==0: continue
        lsp = [] 
        for ln in item['certificate']:
            if ln['professional']=="给排水":ln['professional'] = "给水排水"
            lsp.append(ln)
        lsUpdate.append([{'id':item['id']}, {'$set':{'certificate':lsp}}])
    index = 0
